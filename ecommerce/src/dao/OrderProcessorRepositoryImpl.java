...
public class OrderProcessorRepositoryImpl implements OrderProcessorRepository {
	...
	public List<Product> getAllFromCart(Customer customer) {
		...
		String query = "SELECT p.product_id, p.name, p.price, c.quantity " + "FROM cart c "
	...
	@Override
	public boolean placeOrder(Customer customer, List<Map<Product, Integer>> productsWithQuantity,
		...
		String insertOrderItemsQuery = "INSERT INTO order_items (order_id, product_id, quantity, price) VALUES (?, ?, ?, ?)"; // Added
																																// price
																																// here
		double totalPrice = 0;
				...
				for (Product product : productQuantityMap.keySet()) {
					try (PreparedStatement stmt = connection.prepareStatement(fetchProductPrice)) {
						...
						while (rs.next()) {
							...
							totalPrice += price * productQuantityMap.get(product);
						}
					} catch (SQLException e) {
						e.printStackTrace();
					}
	...
	public List<Map<Product, Integer>> getOrdersByCustomer(int customerId) {
		...
		String query = "SELECT oi.order_id, oi.product_id, oi.quantity, p.name, p.price, p.description "
				...
				+ "JOIN orders o ON o.order_id = oi.order_id " + "WHERE o.customer_id = ?";
		...
		try (PreparedStatement stmt = connection.prepareStatement(query)) {
			...
			while (rs.next()) {
				Product product = new Product(rs.getInt("product_id"), rs.getString("name"), rs.getDouble("price"),
						rs.getString("description"), 0 // Stock quantity is not needed in the order details
				);
	...
	public Product getProductById(int productId) throws ProductNotFoundException {
	    ...
	    try (Connection conn = DBConnUtil.getConnection();
	        ...
	        e.printStackTrace();
	        throw new ProductNotFoundException("Database error while retrieving product");
	    }
	}
}